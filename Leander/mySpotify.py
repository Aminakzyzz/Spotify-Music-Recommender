...
def searchSong(toSearch):
    ...
    # print(audioFeatures.keys())

    for key, value in audioFeatures.items():
        if key not in ["type", "id", "uri", "track_href", "analysis_url", "time_signature"]:
            dataToReturn[key] = value
        # print(key, value)

    if "time_signature" in dataToReturn:
        print("WHYYYYYYYYYYYY IS THERE TIME SIGNATURE???" + dataToReturn["time_signature"])

    print(dataToReturn.keys())
